...
我理解大家会担心：开启 Pro 外网入口后，服务器会不会看到或收集自己的 Codex 对话、图片、电脑信息。按当前实现，Codex Mini 的定位不是云端聊天服务，A1 服务器只负责把手机请求转发回你自己的 Mac。

当前代码里的边界是：

- **不代替你的 Codex 账号**：Codex 登录状态、线程切换、输入发送、回复读取都在你自己的 Mac 和 Codex Desktop 上完成，服务器不登录、不托管、不运行你的 Codex
- **不保存聊天内容和图片内容**：中转服务只把请求/响应临时转发给你的 Mac，不把聊天正文、图片、Codex 线程内容落库保存
- **不保存本机访问令牌**：用户手机访问本机服务的 token 不会作为管理员后台入口保存；旧的后台远程访问入口已关闭
- **只能看到必要的服务元数据**：为了授权、在线状态和限流，服务器会处理设备 ID、会员计划、在线心跳、流量用量、图片次数/大小限制等必要信息
- **请保护自己的访问链接**：不要把带 token 的网页链接、授权码、电脑隐私信息发给陌生人

简单说：A1 服务器会参与“连通、授权、限流和流量统计”，但当前实现不会把你的聊天正文、图片内容、Codex 会话内容或本机访问 token 保存下来，也没有给管理员保留远程查看你 Mac 的后台入口。
